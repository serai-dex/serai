...
use frost::{algorithm::Algorithm, curve::Ed25519, FrostError, FrostView};
use transcript::{RecommendedTranscript, Transcript};

use crate::{
    frost::{read_dleq, write_dleq, MultisigError},
    ringct::{
...
impl Algorithm<Ed25519> for ClsagMultisig {
    ...
    fn nonces(&self) -> Vec<Vec<dfg::EdwardsPoint>> {
        vec![vec![
            dfg::EdwardsPoint::generator(),
            dfg::EdwardsPoint(self.H),
        ]]
    }
    ...
    fn process_addendum<Re: Read>(
        ...
        if self.image.is_identity().into() {
            ...
            self.transcript
                .append_message(b"mask", &self.mask().to_bytes());
        }

        self.transcript
            .append_message(b"participant", &l.to_be_bytes());
        let image = read_dleq(serialized, self.H, l, view.verification_share(l))
            .map_err(|_| FrostError::InvalidCommitment(l))?
            .0;
        self.transcript
            .append_message(b"key_image_share", image.compress().to_bytes().as_ref());
        self.image += image;
