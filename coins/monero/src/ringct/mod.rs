...
use crate::{
    ringct::{bulletproofs::Bulletproofs, clsag::Clsag},
    serialize::*,
};
...
impl RctPrunable {
    ...
    pub fn signature_serialize<W: std::io::Write>(&self, w: &mut W) -> std::io::Result<()> {
        match self {
            ...
            RctPrunable::Clsag { bulletproofs, .. } => bulletproofs
                .iter()
                .map(|bp| bp.signature_serialize(w))
                .collect(),
        }
    }
}
...
impl RctSignatures {
    ...
    pub fn deserialize<R: std::io::Read>(
        ...
        Ok(RctSignatures {
            base: base.0,
            prunable: RctPrunable::deserialize(base.1, &decoys, r)?,
        })
    }
}
