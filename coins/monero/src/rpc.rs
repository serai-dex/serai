...
fn rpc_hex(value: &str) -> Result<Vec<u8>, RpcError> {
    hex::decode(value)
        .map_err(|_| RpcError::InternalError("Monero returned invalid hex".to_string()))
}
...
impl Rpc {
    ...
    async fn call_tail<Response: DeserializeOwned + Debug>(
        ...
        Ok(if !method.ends_with(".bin") {
            ...
            monero_epee_bin_serde::from_bytes(
                &res.bytes().await.map_err(|_| RpcError::ConnectionError)?,
            )
                ...
                .map(|res| {
                    ...
                    .map_err(|_| {
                        RpcError::InvalidTransaction(
                            hex::decode(&res.tx_hash).unwrap().try_into().unwrap(),
                        )
                    })?;
    ...
    pub async fn get_fee(&self) -> Result<Fee, RpcError> {
        ...
        Ok(Fee {
            per_weight: res.result.fee,
            mask: res.result.quantization_mask,
        })
    }
    ...
    pub async fn publish_transaction(&self, tx: &Transaction) -> Result<(), RpcError> {
        ...
        let res: SendRawResponse = self
            .rpc_call(
                ...
                Some(json!({ "tx_as_hex": hex::encode(&buf) })),
            )
            .await?;
