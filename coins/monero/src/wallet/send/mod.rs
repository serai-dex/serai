...
use monero::{blockdata::transaction::SubField, consensus::Encodable, PublicKey};
...
use crate::{
    random_scalar,
    ringct::{
        bulletproofs::{Bulletproofs, MAX_OUTPUTS},
        clsag::{Clsag, ClsagError, ClsagInput},
        generate_key_image, RctBase, RctPrunable, RctSignatures,
    },
    rpc::{Rpc, RpcError},
    transaction::{Input, Output, Timelock, Transaction, TransactionPrefix},
    wallet::{
        address::{Address, AddressType},
        amount_encryption, commitment_mask, key_image_sort, shared_key, uniqueness, Decoys,
        SpendableOutput,
    },
    Commitment,
};
...
async fn prepare_inputs<R: RngCore + CryptoRng>(
    ...
    signable.sort_by(|x, y| {
        x.1.compress()
            .to_bytes()
            .cmp(&y.1.compress().to_bytes())
            .reverse()
    });
...
impl SignableTransaction {
    ...
    fn prepare_outputs<R: RngCore + CryptoRng>(
        ...
        for (o, output) in self.payments.iter().enumerate() {
            self.outputs
                .push(SendOutput::new(rng, uniqueness, *output, o));
        }
    ...
    fn prepare_transaction(&self, commitments: &[Commitment], bp: Bulletproofs) -> Transaction {
        ...
        let mut extra = vec![];
        SubField::TxPublicKey(PublicKey {
            point: self.outputs[0].R.compress(),
        })
        ...
        SubField::AdditionalPublickKey(
            self.outputs[1..]
                .iter()
                .map(|output| PublicKey {
                    point: output.R.compress(),
                })
                .collect(),
        )
        .consensus_encode(&mut extra)
            ...
            rct_signatures: RctSignatures {
                base: RctBase {
                    ...
                    commitments: commitments
                        .iter()
                        .map(|commitment| commitment.calculate())
                        .collect(),
                },
        ...
        match tx.rct_signatures.prunable {
            ...
            RctPrunable::Clsag {
                ...
                clsags.append(
                    &mut clsag_pairs
                        .iter()
                        .map(|clsag| clsag.0.clone())
                        .collect::<Vec<_>>(),
                );
                pseudo_outs.append(
                    &mut clsag_pairs
                        .iter()
                        .map(|clsag| clsag.1.clone())
                        .collect::<Vec<_>>(),
                );
            }
        }
