...
use monero::{blockdata::transaction::ExtraField, consensus::deserialize};

use crate::{
    serialize::{read_32, read_point, read_scalar, write_varint},
    transaction::{Timelock, Transaction},
    wallet::{amount_decryption, commitment_mask, shared_key, uniqueness, ViewPair},
    Commitment,
};
...
impl Timelocked {
    ...
    pub fn unlocked(&self, timelock: Timelock) -> Option<Vec<SpendableOutput>> {
        ...
        self.0
            .partial_cmp(&timelock)
            .filter(|_| self.0 <= timelock)
            .map(|_| self.1.clone())
    }
