...
contract Router is Schnorr {
    ...
    function execute(
        ...
        for(uint256 i = 0; i < transactions.length; i++) {
                (bool success, bytes memory data) = transactions[i].to.call{value: transactions[i].value}(
                    transactions[i].data
                );
                emit Executed(success, data);
        }
    }
    ...
    function executeNoABIEncode(
        ...
        bytes32 message;
        assembly {
            calldatacopy(0x40, 0, sub(calldatasize(), signatureSize))
            message := keccak256(0x40, sub(calldatasize(), signatureSize))
        }
        require(verify(parity, px, message, s, e));
        for(uint256 i = 0; i < transactions.length; i++) {
                (bool success, bytes memory data) = transactions[i].to.call{value: transactions[i].value}(
