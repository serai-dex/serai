...
impl<C: Curve> SecretShareMachine<C> {
  ...
  fn verify_r1<R: RngCore + CryptoRng>(
    ...
    let mut signatures = Vec::with_capacity(usize::from(self.params.n() - 1));
...
impl<C: Curve> KeyMachine<C> {
  ...
  pub fn complete<R: RngCore + CryptoRng>(
    ...
    Ok(FrostCore { params: self.params, secret_share, group_key: stripes[0], verification_shares })
  }
}
