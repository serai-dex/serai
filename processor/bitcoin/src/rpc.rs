...
#[derive(Clone)]
pub(crate) struct Rpc<D: 'static + Send + Sync + Db> {
  pub(crate) db: D,
  pub(crate) rpc: BRpc,
}

impl<D: 'static + Send + Sync + Db> ScannerFeed for Rpc<D> {
...
impl<D: 'static + Send + Sync + Db> TransactionPublisher<Transaction> for Rpc<D> {
  type EphemeralError = RpcError;
