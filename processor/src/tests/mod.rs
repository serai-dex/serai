...
use crate::{
    coin::{Coin, Monero},
    wallet::{MemCoinDb, Wallet, WalletKeys},
...
impl Network for LocalNetwork {
    async fn round(
        &mut self,
        data: Vec<u8>,
    ) -> Result<HashMap<u16, Cursor<Vec<u8>>>, NetworkError> {
        {
...
async fn test_send<C: Coin + Clone>(coin: C, fee: C::Fee) {
    ...
    for i in 1..=threshold {
        ...
        wallet.add_keys(&WalletKeys::new(
            Arc::try_unwrap(keys.remove(&i).take().unwrap()).unwrap(),
            0,
        ));
        wallets.push(wallet);
    }
