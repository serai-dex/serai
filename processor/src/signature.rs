...
impl SignatureProcess {
  ...
  pub async fn run(self) {
    ...
    consume_pubkey_coordinator(&self.kafka_config, &self.identity, &coin_hashmap);
...
fn consume_pubkey_coordinator(
  ...
  for (_key, value) in hashmap_clone.into_iter() {
    if *value == true {
      ...
      tpl.add_partition(&format!("{}_{}", &identity, _key.to_string().to_lowercase()), 0);
      ...
      tokio::spawn(async move {
        for msg_result in &consumer {
          ...
          if key.to_string().contains(&String::from(message_box::ids::COORDINATOR).to_lowercase()) && coord_pubkey_check.is_err()
          {
...
// Initialize consumer used to read test messages from coordinator on general partition
fn consume_coordinator_general_test_message(
  kafka_config: &KafkaConfig,
  identity: &str,
  coin_hashmap: &HashMap<Coin, bool>,
) {
...
fn send_processor_pubkey(
  ...
  producer
    .send(
      BaseRecord::to(&topic)
        .key(&format!("{}_{}_pubkey", identity, String::from(processor).to_lowercase()))
        .payload(&coin_msg)
        .partition(0),
    )
...
async fn send_general_and_secure_test_message(
  ...
  producer
    .send(
      BaseRecord::to(&topic)
        .key(&format!("{}_{}_general", &identity, &String::from(processor).to_lowercase()))
        .payload(&msg)
        .partition(0),
    )
    .expect("failed to send message");
  ...
  producer
    .send(
      BaseRecord::to(&topic)
        .key(&format!("{}_{}_secure", &identity, &String::from(processor).to_lowercase()))
        .payload(&enc)
        .partition(1),
    )
    .expect("failed to send message");
