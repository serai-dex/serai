...
fn create_admin_client(server: &String) -> AdminClient<DefaultClientContext> {
  create_config(server).create().expect("admin client creation failed")
}
...
impl SignatureProcess {
  ...
  pub async fn run(self) {
    ...
    consume_pubkey_coordinator(&kafka_server, &self.identity);

    // Initialize producer to send processor pubkeys to coordinator on general partition
    produce_processor_pubkey(&kafka_server, &self.identity, &coin_hashmap);
    ...
    consume_coordinator_general_test_message(&kafka_server, &self.identity, &coin_hashmap);

    // Initialize consumer used to read secure test messages from coordinator on secure partition
    consume_coordinator_secure_test_message(&kafka_server, &self.identity, &coin_hashmap);
...
// Initialize consumers to read the coordinator pubkey on general partition
fn consume_pubkey_coordinator(server: &str, identity: &str) {
...
// Initialize consumer used to read test messages from coordinator on general partition
fn consume_coordinator_general_test_message(server: &str, identity: &str, coin_hashmap: &HashMap<Coin, bool>) {
...
// Initialize consumer used to read secure test messages from coordinator on secure partition
fn consume_coordinator_secure_test_message(server: &str, identity: &str, coin_hashmap: &HashMap<Coin, bool>) {
  let hashmap_clone = coin_hashmap.clone();
...
// Initialize producer to send processor pubkeys to coordinator on general partition
fn produce_processor_pubkey(server: &str,identity: &str, coin_hashmap: &HashMap<Coin, bool>) {
  let hashmap_clone = coin_hashmap.clone();
  ...
  for (_key, value) in hashmap_clone.into_iter() {
    if *value == true {
      ...
      send_processor_pubkey(&server, &identity, &topic, env_key.to_string(), processor_id);
    }
  }
}

// Sends processor pubkeys to coordinator on general partition
fn send_processor_pubkey(server: &str, identity: &str, topic: &str, env_key: String, processor: &'static str) {
