...
pub(crate) struct BatchTask<D: 'static + Send + Db, S: ScannerFeed> {
  db: D,
  _S: PhantomData<S>,
}

impl<D: 'static + Send + Db, S: ScannerFeed> BatchTask<D, S> {
...
impl<D: 'static + Send + Db, S: ScannerFeed> ContinuallyRan for BatchTask<D, S> {
