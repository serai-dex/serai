...
*/
pub(crate) struct IndexTask<D: 'static + Send + Db, S: ScannerFeed> {
  db: D,
  feed: S,
}

impl<D: 'static + Send + Db, S: ScannerFeed> IndexTask<D, S> {
...
impl<D: 'static + Send + Db, S: ScannerFeed> ContinuallyRan for IndexTask<D, S> {
  type Error = String;
