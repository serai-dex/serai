...
pub(crate) struct SubstrateTask<D: 'static + Send + Db, S: ScannerFeed> {
  db: D,
  _S: PhantomData<S>,
}

impl<D: 'static + Send + Db, S: ScannerFeed> SubstrateTask<D, S> {
  pub(crate) fn new(db: D) -> Self {
    Self { db, _S: PhantomData }
  }
}

impl<D: 'static + Send + Db, S: ScannerFeed> ContinuallyRan for SubstrateTask<D, S> {
