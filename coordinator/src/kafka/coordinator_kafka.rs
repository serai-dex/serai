...
pub fn kafka_send_msg() {
  ...
  let A_PRIV =
    message_box::PrivateKey::from_string(env::var("COORD_BOX_PRIV").unwrap().to_string());
  let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("COORD_BOX_PUB").unwrap().to_string());

  let B_PRIV = message_box::PrivateKey::from_string(env::var("NODE_BOX_PRIV").unwrap().to_string());
  let B_PUB = message_box::PublicKey::from_trusted_str(&env::var("NODE_BOX_PUB").unwrap().to_string());
