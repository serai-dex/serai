...
pub fn start() {
  println!("Starting XMR Kafka Test");
  ...
  consumer.subscribe(&["coord_to_xmr_topic"]).expect("XMR topic subscribe failed");
  ...
  thread::spawn(move || {
    for msg_result in &consumer {
      ...
      let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("XMR_BOX_PUB").unwrap().to_string());

      let B_PRIV =
        message_box::PrivateKey::from_string(env::var("COORD_BOX_PRIV").unwrap().to_string());

      let mut b_others = HashMap::new();
      b_others.insert("XMR_Processor", A_PUB);

      let b_box = MessageBox::new("Coordinator", B_PRIV, b_others);

      // Decrypt message using Message Box
      let encoded_string = b_box.decrypt_from_str(&"XMR_Processor", &encrypted_msg).unwrap();
...
pub fn kafka_send_msg() {
  ...
  let A_PRIV = message_box::PrivateKey::from_string(env::var("XMR_BOX_PRIV").unwrap().to_string());
  let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("XMR_BOX_PUB").unwrap().to_string());
  ...
  let a_box = MessageBox::new("XMR_Processor", A_PRIV, a_others);
  let b_box = MessageBox::new("Coordinator", B_PRIV, b_others);
  // Creates a producer to send message
  let producer: ThreadedProducer<ProduceCallbackLogger> = ClientConfig::new()
    .set("bootstrap.servers", "localhost:9094")
    .create_with_context(ProduceCallbackLogger {})
    .expect("XMR invalid producer config");

  println!("Sending XMR message to Coordinator");

  // Creates a message & encryptes using Message Box
  let msg = b"Private XMR Message to Coordinator".to_vec();
  let enc = a_box.encrypt_to_string(&"Coordinator", &msg.clone());

  // Sends message to Coordinator
  producer
    .send(BaseRecord::to("xmr_to_coord_topic").key(&format!("xmr-user-{}", 1)).payload(&enc))
    .expect("XMR failed to send message");
...
impl ConsumerContext for ConsumerCallbackLogger {
  ...
  fn post_rebalance<'a>(&self, rebalance: &rdkafka::consumer::Rebalance<'a>) {
    println!("XMR post_rebalance callback");

    match rebalance {
      Rebalance::Assign(tpl) => {
        for e in tpl.elements() {
          println!("XMR rebalanced partition {}", e.partition())
        }
      }
      Rebalance::Revoke(tpl) => {
        println!("XMR ALL partitions have been REVOKED")
      }
      Rebalance::Error(err_info) => {
        println!("XMR Post Rebalance error {}", err_info)
      }
    }
  }
      ...
      Ok(_) => {
        for e in offsets.elements() {
          match e.offset() {
            ...
            Offset::Invalid => {}
            _ => {
              println!("XMR committed offset {:?} in partition {}", e.offset(), e.partition())
            }
          }
        }
      }
      Err(err) => {
        println!("XMR error committing offset - {}", err)
      }
    }
  }
}
  ...
  fn delivery(
    ...
    match dr {
      Ok(msg) => {
        let key: &str = msg.key_view().unwrap().unwrap();
        println!(
          "XMR Produced message with key {} in offset {} of partition {}",
          key,
          msg.offset(),
          msg.partition()
        );
      ...
      Err(producer_err) => {
        ...
        println!("XMR failed to produce message with key {} - {}", key, producer_err.0,)
      }
    }
  }
}
