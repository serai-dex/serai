...
pub fn start() {
  println!("Starting Node Kafka Test");
  let consumer: BaseConsumer<ConsumerCallbackLogger> = ClientConfig::new()
    .set("bootstrap.servers", "localhost:9094")
    .set("group.id", "serai")
    .create_with_context(ConsumerCallbackLogger {})
    .expect("Node invalid consumer config");

  consumer.subscribe(&["coord_to_node_topic"]).expect("Node topic subscribe failed");
  ...
  thread::spawn(move || {
    for msg_result in &consumer {
      ...
      let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("NODE_BOX_PUB").unwrap().to_string());

      let B_PRIV =
        message_box::PrivateKey::from_string(env::var("COORD_BOX_PRIV").unwrap().to_string());

      let mut b_others = HashMap::new();
      b_others.insert("Node_Processor", A_PUB);

      let b_box = MessageBox::new("Coordinator", B_PRIV, b_others);

      // Decrypt message using Message Box
      let encoded_string = b_box.decrypt_from_str(&"Node_Processor", &encrypted_msg).unwrap();
...
pub fn kafka_send_msg() {
  ...
  let A_PRIV = message_box::PrivateKey::from_string(env::var("NODE_BOX_PRIV").unwrap().to_string());
  let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("NODE_BOX_PUB").unwrap().to_string());
  ...
  let a_box = MessageBox::new("Node_Processor", A_PRIV, a_others);
  let b_box = MessageBox::new("Coordinator", B_PRIV, b_others);
  // Creates a producer to send message
  let producer: ThreadedProducer<ProduceCallbackLogger> = ClientConfig::new()
    .set("bootstrap.servers", "localhost:9094")
    .create_with_context(ProduceCallbackLogger {})
    .expect("Node invalid producer config");

  println!("Sending Node message to Coordinator");

  // Creates a message & encryptes using Message Box
  let msg = b"Private Node Message to Coordinator".to_vec();
  let enc = a_box.encrypt_to_string(&"Coordinator", &msg.clone());

  // Sends message to Coordinator
  producer
    .send(BaseRecord::to("node_to_coord_topic").key(&format!("node-user-{}", 1)).payload(&enc))
    .expect("Node failed to send message");
...
impl ConsumerContext for ConsumerCallbackLogger {
  ...
  fn post_rebalance<'a>(&self, rebalance: &rdkafka::consumer::Rebalance<'a>) {
    println!("Node post_rebalance callback");

    match rebalance {
      Rebalance::Assign(tpl) => {
        for e in tpl.elements() {
          println!("Node rebalanced partition {}", e.partition())
        }
      }
      Rebalance::Revoke(tpl) => {
        println!("Node ALL partitions have been REVOKED")
      }
      Rebalance::Error(err_info) => {
        println!("Node Post Rebalance error {}", err_info)
      }
    }
  }
      ...
      Ok(_) => {
        for e in offsets.elements() {
          match e.offset() {
            ...
            Offset::Invalid => {}
            _ => {
              println!("Node committed offset {:?} in partition {}", e.offset(), e.partition())
            }
          }
        }
      }
      Err(err) => {
        println!("Node error committing offset - {}", err)
      }
    }
  }
}
  ...
  fn delivery(
    ...
    match dr {
      Ok(msg) => {
        let key: &str = msg.key_view().unwrap().unwrap();
        println!(
          "Node Produced message with key {} in offset {} of partition {}",
          key,
          msg.offset(),
          msg.partition()
        );
      ...
      Err(producer_err) => {
        ...
        println!("Node failed to produce message with key {} - {}", key, producer_err.0,)
      }
    }
  }
}
