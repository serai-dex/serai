...
pub fn start() {
  println!("Starting ETH Kafka Test");
  ...
  consumer.subscribe(&["coord_to_eth_topic"]).expect("ETH topic subscribe failed");
  ...
  thread::spawn(move || {
    for msg_result in &consumer {
      ...
      let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("ETH_BOX_PUB").unwrap().to_string());

      let B_PRIV =
        message_box::PrivateKey::from_string(env::var("COORD_BOX_PRIV").unwrap().to_string());

      let mut b_others = HashMap::new();
      b_others.insert("ETH_Processor", A_PUB);

      let b_box = MessageBox::new("Coordinator", B_PRIV, b_others);

      // Decrypt message using Message Box
      let encoded_string = b_box.decrypt_from_str(&"ETH_Processor", &encrypted_msg).unwrap();
...
pub fn kafka_send_msg() {
  ...
  let A_PRIV = message_box::PrivateKey::from_string(env::var("ETH_BOX_PRIV").unwrap().to_string());
  let A_PUB = message_box::PublicKey::from_trusted_str(&env::var("ETH_BOX_PUB").unwrap().to_string());
  ...
  let a_box = MessageBox::new("ETH_Processor", A_PRIV, a_others);
  let b_box = MessageBox::new("Coordinator", B_PRIV, b_others);
  // Creates a producer to send message
  let producer: ThreadedProducer<ProduceCallbackLogger> = ClientConfig::new()
    .set("bootstrap.servers", "localhost:9094")
    .create_with_context(ProduceCallbackLogger {})
    .expect("ETH invalid producer config");

  println!("Sending ETH message to Coordinator");

  // Creates a message & encryptes using Message Box
  let msg = b"Private ETH Message to Coordinator".to_vec();
  let enc = a_box.encrypt_to_string(&"Coordinator", &msg.clone());

  // Sends message to Coordinator
  producer
    .send(BaseRecord::to("eth_to_coord_topic").key(&format!("eth-user-{}", 1)).payload(&enc))
    .expect("ETH failed to send message");
...
impl ConsumerContext for ConsumerCallbackLogger {
  ...
  fn post_rebalance<'a>(&self, rebalance: &rdkafka::consumer::Rebalance<'a>) {
    println!("ETH post_rebalance callback");

    match rebalance {
      Rebalance::Assign(tpl) => {
        for e in tpl.elements() {
          println!("ETH rebalanced partition {}", e.partition())
        }
      }
      Rebalance::Revoke(tpl) => {
        println!("ETH ALL partitions have been REVOKED")
      }
      Rebalance::Error(err_info) => {
        println!("ETH Post Rebalance error {}", err_info)
      }
    }
  }
      ...
      Ok(_) => {
        for e in offsets.elements() {
          match e.offset() {
            ...
            Offset::Invalid => {}
            _ => {
              println!("ETH committed offset {:?} in partition {}", e.offset(), e.partition())
            }
          }
        }
      }
      Err(err) => {
        println!("ETH error committing offset - {}", err)
      }
    }
  }
}
  ...
  fn delivery(
    ...
    match dr {
      Ok(msg) => {
        let key: &str = msg.key_view().unwrap().unwrap();
        println!(
          "ETH Produced message with key {} in offset {} of partition {}",
          key,
          msg.offset(),
          msg.partition()
        );
      ...
      Err(producer_err) => {
        ...
        println!("ETH failed to produce message with key {} - {}", key, producer_err.0,)
      }
    }
  }
}
