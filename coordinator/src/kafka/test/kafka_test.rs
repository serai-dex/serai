...
pub fn start() {
  ...
  consumer.subscribe(&["kakfa_test_b"]).expect("topic subscribe failed");

  // let consumer2: BaseConsumer<ConsumerCallbackLogger> = ClientConfig::new()
  //   .set("bootstrap.servers", "localhost:9094")
  //   .set("group.id", "consumer-group-2")
  //   .create_with_context(ConsumerCallbackLogger {})
  //   .expect("invalid consumer config");

  // consumer2.subscribe(&["kakfa_test_a"]).expect("topic subscribe failed");

  // let consumer3: BaseConsumer<ConsumerCallbackLogger> = ClientConfig::new()
  //   .set("bootstrap.servers", "localhost:9094")
  //   .set("group.id", "consumer-group-2")
  //   .create_with_context(ConsumerCallbackLogger {})
  //   .expect("invalid consumer config");

  // consumer3.subscribe(&["kakfa_test_a"]).expect("topic subscribe failed");
  ...
  producer
    .send(BaseRecord::to("kafka_test_b").key(&format!("user-{}", i)).payload(&enc))
    .expect("failed to send message");
  }

  // producer
  //   .send(BaseRecord::to("kafka_topic").key(&format!("user-{}", 2)).payload(&enc).partition(1))
  //   .expect("failed to send message");
