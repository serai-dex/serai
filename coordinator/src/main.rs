...
async fn main() {
  ...
  // Load identity arg
  let name_arg = args.value_of("name").unwrap().to_lowercase();
  ...
  let core_name_arg = name_arg.to_string();
  ...
  let sig_name_arg = name_arg.to_string();
  tokio::spawn(async move {
  ...
  let observer_name_arg = name_arg.to_string();
  ...
  let network_name_arg = name_arg.to_string();
