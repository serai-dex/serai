...
//! pipeline. State is injected via the Serai node mock (`serai_mock_rpc`), exercising
//! the same `pub` API surface a real coordinator would use.

use rand_core::{CryptoRng, RngCore};
use crate::{tests::*, *};

fn drop_handles(cosigning: &mut Cosigning<MemDb>) {
  for handle in cosigning.join_handles.drain(..) {
    handle.abort();
  }
}

/// Drain all pending cosign intents from every keyed session the fuzzer knows about.
fn drain_intents<R: RngCore + CryptoRng>(
  txn: &mut impl DbTxn,
  event_fuzzer: &BlockEventsFuzzer<R>,
  ...
  for network in ExternalNetworkId::all() {
    ...
    for session_num in 0 .. max_session {
      ...
      for intent in Cosigning::<MemDb>::all_intended_cosigns_for_network(txn, set) {
...
fn sign_and_intake<R: RngCore + CryptoRng>(
  db: &MemDb,
  ...
  let Some(global_cosigning_session) =
    crate::GlobalCosigningSessions::get(db, intent.global_cosigning_session)
  else {
    return Err((network, intent));
  };
  let Some(public) = global_cosigning_session.keys.get(&network) else { return Ok(()) };
...
async fn run_honest_cosigning<R: RngCore + CryptoRng>(
  db: &MemDb,
  cosigning: &mut Cosigning<MemDb>,
  event_fuzzer: &BlockEventsFuzzer<R>,
...
/// with the fuzzer's keypairs, intake them, and repeat until `should_break` returns `true`.
///
...
async fn run_honest_cosigning_capped<R: RngCore + CryptoRng>(
  db: &MemDb,
  cosigning: &mut Cosigning<MemDb>,
  event_fuzzer: &BlockEventsFuzzer<R>,
...
///
/// Uses the `BlockEventsFuzzer` to generate random blocks, spawns the full `Cosigning` pipeline,
...
///
/// The mock Serai RPC has a random failure rate enabled so that RPC calls from the intend task
...
async fn full_stack_fuzzed() {
  ...
  for i in 1 ..= iterations {
    let num_blocks = rng.gen_range(5 .. 20);
    let mut event_fuzzer = BlockEventsFuzzer::new(rng.clone());
    ...
    mock_serai.set_failure_rate(event_fuzzer.rng.gen_range(5 ..= 30)).await;

    let db = MemDb::new();

    let (request, _calls) = TestRequest::new(false);
    let mut cosigning = Cosigning::spawn_with_handles(db.clone(), serai, request, vec![]);

    let target = num_blocks - 1;
...
///
/// Mirrors `full_stack_fuzzed`, random events via `BlockEventsFuzzer`, full `Cosigning` pipeline
...
async fn equivocation_halts_protocol() {
  ...
  for iteration in 1 ..= iterations {
    ...
    let equivocation_after_block: u64 = event_fuzzer.rng.gen_range(2 ..= target / 2);
    ...
    // We need at least one global cosigning session to exist and at least one block
    // cosigned under it.
    ...
    if !reached_equivocation_point {
      serai_env::log::info!(
        "equivocation fuzz ({iteration}/{iterations}): no global cosigning session formed, skipping"
      );
      continue;
    }

    assert!(
      FaultedCosigningSession::get(&db).is_none(),
      "should not be faulted before equivocation"
    );

    // Step 2: inject equivocation

    let equivocation_block = equivocation_after_block;
    let indexed_hash = crate::SubstrateBlockHash::get(&db, equivocation_block)
      .expect("equivocation block should be indexed");

    // Find the global cosigning session that covers this block via the evaluator's current session
    let Some(global_cosigning_session_id) =
      crate::evaluator::current_global_cosigning_session_evaluating(&db)
    else {
      serai_env::log::info!(
        "equivocation fuzz ({iteration}/{iterations}): no evaluated global cosigning \
         session, skipping"
      );
      continue;
    };
    let global_cosigning_session =
      crate::GlobalCosigningSessions::get(&db, global_cosigning_session_id)
        .expect("evaluated session should exist in DB");
    if equivocation_block < global_cosigning_session.start_block_number {
      serai_env::log::info!(
    ...
    let mut faulty_block_hash = random_block_hash(&mut event_fuzzer.rng);
    if faulty_block_hash == indexed_hash {
      faulty_block_hash = random_block_hash(&mut event_fuzzer.rng);
    }

    let fault_threshold = (global_cosigning_session.total_stake * 17) / 100;
    let total_faulty = global_cosigning_session.keys.len();
    // Submit equivocating cosigns one at a time, tracking cumulative fault weight.
    let mut cumulative_faulty_stake: u64 = 0;

    // All networks in the session equivocate, ensuring the 17% fault threshold is always met.
    for (i, (faulty_network, _)) in global_cosigning_session.keys.iter().enumerate() {
      let faulty_cosign = Cosign {
        global_cosigning_session: global_cosigning_session_id,
        block_number: equivocation_block,
        block_hash: faulty_block_hash,
        cosigner: *faulty_network,
      };
      let public = global_cosigning_session
        .keys
        .get(faulty_network)
        .expect("faulty network not in global cosigning session");
      let keypair =
      ...
      let net_stake = global_cosigning_session.stakes.get(faulty_network).copied().unwrap_or(0);
      cumulative_faulty_stake += net_stake;
      let faulted_now = cumulative_faulty_stake >= fault_threshold;

      serai_env::log::info!(
        "faulty cosign {}/{} from {faulty_network:?} (stake={net_stake}): \
      ...
      if faulted_now {
        assert_eq!(
          FaultedCosigningSession::get(&db),
          Some(global_cosigning_session_id),
          "session should be faulted after {faulty_network:?}: cumulative stake \
           {cumulative_faulty_stake} >= threshold {fault_threshold}"
        );
      } else {
        assert!(
          FaultedCosigningSession::get(&db).is_none(),
          "session should NOT be faulted after {faulty_network:?}: cumulative stake \
           {cumulative_faulty_stake} < threshold {fault_threshold}"
        );
      }
    }
    ...
    {
      let mut txn = db.txn();
      drain_intents(&mut txn, &event_fuzzer);
      txn.commit();
    }
...
async fn dos_stall_offline_set() {
  ...
  for iteration in 1 ..= iterations {
    ...
    let num_blocks = rng.gen_range(10 .. 25);
    let mut event_fuzzer = BlockEventsFuzzer::new(rng.clone());
    ...
    let mid = usize::try_from(num_blocks / 2).unwrap();
    if blocks[mid ..].iter().all(Vec::is_empty) {
      #[expect(clippy::as_conversions, clippy::cast_possible_truncation)]
      let burn_index =
        mid + (event_fuzzer.rng.next_u64() as usize % (usize::try_from(num_blocks).unwrap() - mid));
    ...
    // Step 1: honest cosigning until we have a global cosigning session to analyze.
    ...
    // Must be >= 4 so that at least the first block of the first global cosigning session
    // (start_block_number = 4, declared by the forced keygen at block 3) is not
    // deferred. If block 4's intents were deferred, the evaluator could never
    // commit `CurrentlyEvaluatedGlobalSession`, and the test would panic below.
    let step1_low = 4u64;
    let step1_high = target / 3;
    let step1_target: u64 = if step1_low <= step1_high {
      event_fuzzer.rng.gen_range(step1_low ..= step1_high)
    } else {
      step1_low
    };
    ...
    // Find the current global cosigning session and identify a network to take offline.
    // The evaluator runs in a background task, so there is a tiny race between
    // run_honest_cosigning_capped detecting `latest >= step1_target` and the
    // evaluator's transaction that set `CurrentGlobalCosigningSessionEvaluator`
    // becoming visible to this test thread.  Poll with a timeout instead of
    // unwrapping.
    let global_cosigning_session_id = {
      let deadline = std::time::Instant::now() + Duration::from_secs(10);
      let mut last_error;
      loop {
        match crate::evaluator::current_global_cosigning_session_evaluating(&db) {
          Some(id) => break id,
          None => {
            last_error = Some(format!(
              "no current session yet (latest={:?})",
              Cosigning::<MemDb>::latest_cosigned_block_number(&db),
            ));
            assert!(
              std::time::Instant::now() < deadline,
              "timed out waiting for evaluator to establish a current session: {}",
              last_error.unwrap(),
            );
            tokio::time::sleep(Duration::from_millis(50)).await;
          }
    ...
    let global_cosigning_session =
      crate::GlobalCosigningSessions::get(&db, global_cosigning_session_id).unwrap();
    let threshold = crate::evaluator::cosign_threshold(global_cosigning_session.total_stake);

    // Find a network whose absence prevents reaching the threshold
    let (&offline_network, &offline_stake) = global_cosigning_session
      .stakes
      .iter()
      .find(|(_, &stake)| global_cosigning_session.total_stake - stake < threshold)
      .unwrap();
    let online_weight: u64 = global_cosigning_session
      .stakes
      .iter()
    ...
    let stakes_summary: Vec<_> =
      global_cosigning_session.stakes.iter().map(|(net, &s)| format!("{net:?}={s}")).collect();
    ...
    assert!(FaultedCosigningSession::get(&db).is_none());
    ...
    assert!(FaultedCosigningSession::get(&db).is_none(), "absence is not equivocation");
