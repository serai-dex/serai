	...
	T::Assets: Assets<T::AccountId>,
{
		...
		T::MultiAssetIdConverter::try_convert(asset)
	{
		...
		assert_ok!(T::Assets::mint(asset_id, &caller, INITIAL_ASSET_BALANCE.into()));
	}
...
benchmarks! {
	where_clause {
		where
			T::AssetBalance: From<u64> + Into<u64>,
			T::Currency: Currency<T::AccountId>,
	...
	create_pool {
		...
		let asset2: T::MultiAssetId = T::BenchmarkHelper::asset_id(0).into();
	...
	remove_liquidity {
		...
		let total_supply = <T::PoolAssets as LiquidityTokens<T::AccountId>>::total_issuance(lp_token.clone());
	}: _(SystemOrigin::Signed(caller.clone()), asset1, asset2, remove_lp_amount.into(), 0.into(), 0.into(), caller.clone())
	verify {
		let new_total_supply = <T::PoolAssets as LiquidityTokens<T::AccountId>>::total_issuance(lp_token.clone());
		assert_eq!(
			new_total_supply,
	...
	swap_exact_tokens_for_tokens {
		...
		let asset1: T::MultiAssetId = T::BenchmarkHelper::asset_id(1).into();
		let asset2: T::MultiAssetId = T::BenchmarkHelper::asset_id(2).into();
		...
		if !T::AllowMultiAssetPools::get() {
			...
			let asset3: T::MultiAssetId = T::BenchmarkHelper::asset_id(3).into();
	...
	}: _(SystemOrigin::Signed(caller.clone()), path.clone(), 100.into(), (1000 * ed).into(), caller.clone())
	verify {
		if !T::AllowMultiAssetPools::get() {
