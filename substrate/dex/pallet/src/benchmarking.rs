...
benchmarks! {
  ...
  verify {
    let pool_id = (asset1.clone(), asset2.clone());
    let lp_minted = AssetConversion::<T>::calc_lp_amount_for_zero_supply(
      &add_amount.into(),
      &1000.into()
    ).unwrap().into();
    assert_eq!(
    ...
    assert_eq!(
      T::Assets::balance(
        T::BenchmarkHelper::asset_id(0),
        &AssetConversion::<T>::get_pool_account(&pool_id)
      ),
      1000.into()
    );
  }
  ...
  remove_liquidity {
    ...
    let total_supply =
      <T::PoolAssets as LiquidityTokens<T::AccountId>>::total_issuance(lp_token.clone());
  }: _(
    SystemOrigin::Signed(caller.clone()),
    asset1,
    asset2,
