...
pub mod pallet {
  ...
  #[pallet::config]
  pub trait Config: frame_system::Config<AccountId = Public> + DexConfig<Balance = SubstrateAmount, PoolAssetId = u32> {
  ...
  pub type Balances<T: Config> =
    StorageDoubleMap<_, Blake2_128Concat, Public, Blake2_128Concat, u32, SubstrateAmount, OptionQuery>;
  ...
  impl<T: Config> Pallet<T> {
    ...
    pub fn mint_into(token: u32, to: T::AccountId, amount: SubstrateAmount) -> Result<(), Error<T>> {
      ...
      // update the supply
      let new_supply = Self::supply(token)
        .checked_add(amount)
    ...
    pub fn burn_from(token: u32, from: T::AccountId, amount: SubstrateAmount) -> Result<(), Error<T>> {
  ...
  impl<T: Config> LiquidityTokens<T::AccountId> for Pallet<T> {
    ...
    fn mint_into(token: Self::AssetId, to: &Public, amount: Self::Balance) -> Result<Self::Balance, DispatchError> {
      Self::mint_into(token, *to, amount)?;
      Ok(amount)
    }

    fn burn_from(token: Self::AssetId, from: &Public, amount: Self::Balance) -> Result<Self::Balance, DispatchError> {
