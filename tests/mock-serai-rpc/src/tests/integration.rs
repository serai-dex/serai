//! Integration tests for the mock Serai RPC node.
...
// use serai_mock_rpc::{*, events::validator_sets::*};

// #[tokio::test]
// async fn test_basic_block_and_number() {
//   let mock_serai = MockSeraiRpcBuilder::new()
//     .with_block(vec![vec![]])
//     .with_block(vec![vec![]])
//     .with_block(vec![vec![]])
//     .build()
...
//   let mock_serai = MockSeraiRpcBuilder::new().with_block(vec![vec![]]).build().await;
//   let serai = Serai::new(mock_serai.url()).unwrap();
...
//   let mock_serai = MockSeraiRpcBuilder::new().with_block(events.clone()).build().await;
...
//   let mock_serai = MockSeraiRpc::builder().build().await;
...
//   let hash = mock_serai.add_block_with_events(vec![vec![]]).await.0;
...
//   mock_serai.add_block_with_events(vec![vec![]]).await;
//   let latest = serai.latest_finalized_block_number().await.unwrap();
...
//   mock_serai.set_error("blockchain/latest_finalized_block_number", "simulated failure").await;
...
//   mock_serai.clear_error("blockchain/latest_finalized_block_number").await;
...
//   mock_serai.set_error("blockchain/latest_finalized_block_number", "err1").await;
//   mock_serai.set_error("blockchain/block", "err2").await;
...
//   let mock_serai = MockSeraiRpc::builder().with_block(vec![vec![]]).build().await;

//   {
//     let state = mock_serai.state().read().await;
//     assert!(state.published_transactions.is_empty(), "no transactions should exist initially");
//   }

//   {
//     let mut state = mock_serai.state().write().await;
